...
export default function PreferencesModal() {
  ...
  const [sessionRecording, setSessionRecordingData] = useState(true);
  ...
  useEffect(() => {
    ...
    const sessionRecording = window.localStorage.getItem("MV_SESSION_RECORDING")
    setSessionRecordingData(sessionRecording)
  ...
  const handleSettingChange = async (event) => {
    ...
    try {
      ...
      await fetch(`${SERVER_BASE_URL}/${settingName}?value=${value}`, {
        method: "GET",
      });
      posthog.capture(`user requested ${settingName} change succeeded`, {
        value,
      });
    } catch (error) {
      posthog.capture(`user requested ${settingName} change failed`, {
        value,
        error,
      });
  ...
  const handleSessionRecordingSwitchOnChange = async (event) => {
    const value = event.target.checked;
    try {
      await fetch(`${SERVER_BASE_URL}/enable-session-recording?value=${value}`, {
        method: "GET",
      });
      window.localStorage.setItem("MV_SESSION_RECORDING", value);
    } catch (error) {
      console.log(`GET /enable-session-recording failed with error: ${error}`);
    }

    setSessionRecordingData(value);
  };
  ...
  return (
    <div className="ms-auto">
      ...
      <Modal
        contentClassName="preference-panel-content"
        show={show}
        onHide={handleClose}
        animation={false}
        centered
      >
        ...
        <Modal.Body>
          <div className="slider-setting">
            <Form.Label>
              Noise Supression:{" "}
              {Number(settings["noise-suppression-threshold"] || 0).toFixed(1)}
            </Form.Label>
          ...
          <div className="slider-setting">
            ...
            <Form.Label>
              Latency: {Number(settings["callback-latency-ms"]) * 2}ms
            </Form.Label>
            <div className="slider-with-min-max">
              <span>min</span>
              <Form.Range
            ...
            <OverlayTrigger
              ...
              overlay={
                <Tooltip id={`tooltip-right`}>
                  By clicking send, you agree to share your data with MetaVoice
                  striclty for the purposes of providing you a better voice
                  &#38; app experience.
                </Tooltip>
              }
            >
          ...
          <div className="d-flex">
            <Form.Check
              ...
              label="Opt-in to share data"
              checked={shareData}
              onChange={handleSessionRecordingSwitchOnChange}
              className="me-1"
            />
            <OverlayTrigger
              placement="right"
              overlay={
                <Tooltip id={`tooltip-right`}>
                  Enable recording and playback of voice conversion
                </Tooltip>
              }
            >
              <i className="bi bi-info-circle-fill"></i>
        ...
        <div className="d-flex mt-3 justify-content-center">
          <Button variant="danger" onClick={logout} className="col-3">
            Sign Out
          </Button>
        </div>
        <div className="d-flex mt-3 justify-content-center">
