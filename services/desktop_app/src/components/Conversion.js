...
export default function Conversion({ email, issuer }) {
  ...
  const setupIntercom = (options = {}) => {
    return new Promise(async (resolve, reject) => {
      try {
        loadIntercom();

        // calculate user-hash via email
        let userHash = window.localStorage.getItem(USER_HASH_KEY);
        if (!userHash) {
          const response = await fetch(
            `${SERVER_BASE_URL}/intercom-user-hash?email=${options.email}`,
            { method: "GET" }
          );
          userHash = (await response.json()).userHash;
          window.localStorage.setItem(USER_HASH_KEY, userHash);
        }

        bootIntercom({ ...options, user_hash: userHash });
        resolve();
      } catch (error) {
        reject(error);
      }
