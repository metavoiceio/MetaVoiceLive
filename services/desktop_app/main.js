...
ipcMain.handle("request-user-speakers", async (event, ...args) => {
  ...
  try {
    ...
    console.error(
      "Error accessing user data speakers path, will revert to base speakers:",
      err
    );
  }
...
if (!IS_DEV || process.env.DEBUG_OTA === "true") {
  try {
    ...
    console.error(
      "Error setting up updates, probably no internet, will continue without updates"
    );
    console.log(e);
  }
}
...
function createFrontendServer() {
  ...
  portPromise.then((port) => {
    frontendServerApp = frontendApp.listen(port, () => {
      console.log(
        `Localhost frontend server listening at http://localhost:${port}`
      );
    });
  });
}
...
function setupUpdates(obj) {
  ...
  // electron
  try {
    const alreadyInstalledNewVersionLocation = fs.readFileSync(
      path.join(__dirname, "use-new-version.txt"),
      "utf-8"
    );
    if (alreadyInstalledNewVersionLocation) {
      ...
      try {
        ...
        const newAppPath = path.join(
          alreadyInstalledNewVersionLocation,
          `MetaVoice.${process.platform === "darwin" ? "app" : "exe"}`
        );
        ...
        exitFrontendServer()
          ...
          .catch((e) => {
            logError(
              "el update: new version was installed, but could not be opened due to the frontend server being uncloseable",
              e
            );
          });
        ...
        logError(
          "el update: new version was installed, but could not be opened",
          e
        );
        log(
          "el update: if you think there was mistake while updating, please remove `resources/app/use-new-versbion.txt`, and restart this app. This will re-install the new version"
        );
        notify({
          title: "MetaVoice update",
  ...
  fetch(elFeed)
    ...
    .then((update) => {
      if (!update) {
        log(
          "el update: no new versions available. Please don't close the window, mvml updates may be processing"
        );
        return;
      }
      ...
      const downloadDestination = path.resolve(
        `${app.getPath("downloads")}/${pkgName}.zip`
      );
      const destination = path.resolve(
        `${app.getPath("downloads")}/${pkgName}`
      );
      ...
      log(
        `el update: new version "${name}" available! Downloading from ${url} into ${downloadDestination}`
      );
      ...
      if (fs.existsSync(downloadDestination)) {
        log(
          `el update: file already exists at ${downloadDestination}, will remove and re-download`
        );
        fs.rmSync(downloadDestination);
      }

      const fileStream = fs.createWriteStream(downloadDestination, {
        flags: "wx",
      });
      ...
      fileStream.on("finish", () => {
        ...
        const stream = fs
          .createReadStream(downloadDestination)
        ...
        unzipPromise.catch((err) => {
          ...
          log(
            "el update: fatal error decompressing the electron app, please report this to gm@themetavoice.xyz",
            err
          );
        });
        ...
        const markAsOutdatedPromise = unzipPromise.then(() => {
          ...
          const useNewVersionPath = path.resolve(
            `${__dirname}/use-new-version.txt`
          );
        ...
        markAsOutdatedPromise.catch((err) => {
          logError(
            "el update: error marking new version as outdated, please report this to gm@themetavoice.xyz",
            err
          );
        });
        ...
        const _endPromise = markAsOutdatedPromise.then(() => {
          ...
          log(
            `el update: success! Please open the executable in ${destination} to use the new version! Feel free to delete this version`
          );
        });
      });
    });
...
function handleCustomProtocol(url) {
  ...
  console.log(
    "Loading:",
    "http://localhost:3000/" + url.slice("metavoice://".length)
  );
  mainWindow.loadURL(
    "http://localhost:3000/" + url.slice("metavoice://".length)
  );
