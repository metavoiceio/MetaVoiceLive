...
const { app, BrowserWindow, shell, systemPreferences, ipcMain, protocol, session } = require('electron');
...
if (!IS_DEV || process.env.DEBUG_SHORTCUT === 'true') {
  try {
    setupShortcuts();
  } catch (e) {
    console.error('Error setting up shortcuts');
    console.log(e);
  }
}
...
function setupShortcuts() {
  if (appMode === 'update') {
    console.log('shortcut: skipping shortcut setup in update mode');
    return;
  }
  if (process.platform !== 'win32') return;

  // add a shortcut icon to user's desktop pointing to the executable
  const shortcutPath = path.resolve(`${app.getPath('desktop')}/MetaVoice Live.lnk`);
  const exePath = path.resolve(`${__dirname}/../../MetaVoice.exe`);
  const shortcutOpts = {
    target: exePath,
    description: "MetaVoice Live",
    // TODO icon? Need to bundle properly in exe
  }
  let success = false;
  if (!fs.existsSync(shortcutPath)) {
    // create the shortcut
    console.log(`shortcut: creating shortcut at ${shortcutPath} to ${exePath}`);
    success = shell.writeShortcutLink(shortcutPath, 'create', shortcutOpts)
  } else {
    // update the shortcut to point to the the current exe
    console.log(`shortcut: updating shortcut at ${shortcutPath} to point to ${exePath}`);
    success = shell.writeShortcutLink(shortcutPath, 'update', shortcutOpts)
  }
...
function handleCustomProtocol(url) {
  ...
  // we only want the tokens stored in the hash, the rest of the url can be ignored
  const hash = url.replace(/^.+#/, "#");

  const port = IS_DEV ? 3000 : (frontendServerApp?.address()?.port ?? 3000);
  const internalUrl = `http://localhost:${port}/${hash}`;

  mainWindow.loadURL(internalUrl);
  setTimeout(() => {
    // reload page so supabase knows to use the hash tokens
    mainWindow.reload();
    // but wait so the page can receive the hash
  }, 1000)
}
...
if (!gotTheLock) {
  ...
  app.whenReady().then(() => {
    ...
    // workaround for email+password sign-in: supabase gets the token, but no hash is set,
    // so no hash is detected, so the user is not logged in. But refreshing the page fixes it.
    const SUPABASE_URL = 'https://rxhakgjibqkojyocfpjt.supabase.co/auth/v1/token?*'
    session.defaultSession.webRequest.onCompleted({ urls: [SUPABASE_URL] }, (details) => {
      // only reload if auth succeeded, otherwise no error message would be shown
      if (details.statusCode === 200) {
        // wait for supabase to set internals correctly
        setTimeout(() => {
          mainWindow.reload();
        }, 500)
      }
